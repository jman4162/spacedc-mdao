...
**Educational white paper.** `docs/whitepaper.md` ("Orbital data centers, by the numbers") is a ~8-page primer teaching the technical + economic metrics through reproducible package numbers, built on the Project Suncatcher paper (arXiv:2511.19468) with Economist/McCalip/Starcloud context. In the mkdocs nav + a README pointer. A downloadable PDF is built by `scripts/build_whitepaper_pdf.py` (a stdlib-only Markdown-to-LaTeX converter compiled with **Tectonic**, the self-contained XeTeX engine — `brew install tectonic`; no pandoc/LaTeX distro or Python deps) to `docs/assets/orbital-data-centers-primer.pdf` (committed). `scripts/generate_readme_assets.py` also emits a static `viability_ladder.png` for the paper/PDF. All numbers are pulled from live model runs. Note: a full `rm -rf .venv && uv sync` was needed once this session after a `uv sync --reinstall` left numpy/pydantic half-installed — prefer `--reinstall-package <name>` over a blanket `--reinstall`.

**Streamlit Cloud showcase app.** `streamlit_app.py` (repo root, the entry point) + an `app/` package: `app/data.py` (pure model glue — scenario discovery, override-aware `evaluate_space`/`evaluate_earth` runs, baselines for sliders, binding constraints, thresholds, tornado/Monte Carlo; no streamlit/plotly imports so it stays mypy-strict and importable under base deps), `app/theme.py` (dark `plotly_dark` template), `app/tabs.py` (four tab renderers reusing `orbitdc.viz.plotly_figures` + `viz.provenance.collect_provenance`). Sidebar sliders build an `overrides` dict threaded through `evaluate_space`; defaults equal the scenario baseline so an untouched app reproduces `compare()`. Tornado/Monte Carlo run around the selected-scenario baseline (no overrides seam on `ComparisonResult`). Runtime is the slim `[app]` extra (`streamlit`, `plotly`, `networkx` — no panel/kaleido/mdao) mirrored in repo-root `requirements.txt` (installs the package via `.`), plus `.streamlit/config.toml` (dark theme). `tests/test_streamlit_app.py` guards the `app.data` contract headless (pytest `pythonpath = ["."]` so `app` imports); mypy `files` gained `app` with a `disallow_untyped_calls=false` override for `app.tabs`/`app.theme`. Run: `uv run --extra app streamlit run streamlit_app.py`. Verified end-to-end via `streamlit.testing.v1.AppTest` (4 tabs, zero exceptions across the orbital scenarios + earth-baseline switching). Deploy is a one-time manual step (connect the repo on Streamlit Community Cloud, then set the live URL in README/`docs/streamlit-app.md`). Docs page in the mkdocs nav + README "Live demo" pointer.
